...
use chrono::TimeZone;
use olly_db::models::repository::{DieselRepository, Repository};
...
pub async fn post(Json(payload): Json<TracingPayload>) -> (StatusCode, Json<()>) {
    ...
    let mut repo = DieselRepository {
        connection: &mut conn,
        table: olly_db::schema::span::table,
    };
    ...
    fn process_span(
        span: Span,
        repo: &mut DieselRepository<olly_db::schema::span::table>,
        ...
        ) = (start_time, end_time)
        {
            ...
            let insertable_span = olly_db::models::span::InsertableSpan {
